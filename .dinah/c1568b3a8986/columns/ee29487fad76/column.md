...
The diff review happens on the card's pull request. The arriving comment carries a `[PR #<n>](<url>)` Markdown link, so the operator navigates straight to the diff in GitHub's interface or a VS Code pull-request tool, reads it there, and records change-requests on the PR itself. The workbench mirrors the verdict: acceptance moves the card forward, change-requests move it back to Implement with the requests summarised in a comment. A code card arriving without a PR link is an incomplete handoff; Agent Code Review was told to catch it, and the fallback is `gh pr view <branch>` against the card's recorded branch.

PRs on this repository are authored by the operator's own account, because the pipeline bot has no write access here. GitHub will not let an author approve their own pull request, so the Approve button is unavailable and the verdict lives on the card rather than on the PR. Read the diff, then say in a comment what you decided. Restoring the Approve button means granting the bot write access, which is the operator's call and is recorded in the workbench body.
