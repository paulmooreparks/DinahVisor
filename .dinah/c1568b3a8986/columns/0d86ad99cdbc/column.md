...
kind: work
---
Fresh-context review of the spec by a review agent. The operator is not in this loop for spec quality; they hold the next station, Operator Design Review, and what you pass forward lands in front of them.

Exit by exactly one, in priority order:

1. **Push back to Spec** when the contract is structurally unsound: wrong, contradictory, untestable, or scope-crept beyond the card body's framing. Release your claim, then `dinah move` the card to Spec so it lands ready for whoever picks it up.
2. **Block the card where it sits** when the spec is contested or undeliverable without a ruling nobody anticipated: the framing itself is in dispute, a sustained review loop, or a question whose answer decides what the card even is. Write `dinah block <card> --kind operator_decision --reason "<what needs deciding, posed so the operator can answer it without opening the card>"`. The block frees the card and halts it here, and only the operator lifts it, so do not move the card afterwards. Never park such a card at Implement.
3. **Move forward to Operator Design Review** in every other case; the route places that station directly after this one, and no card goes around it to Implement. Post a comment that tells the operator what awaits them: the pending owner operator questions listed, the one-line acceptance criterion an artifact's approval would create ("the shipped page matches the approved sketch on the card's branch"), or the plain statement that the card is clean so one glance sends it on. A pending operator question must never ride the flow past that station.

### Telling exit 2 from exit 3

Both end at the operator and they mean different things. A block says work stopped on a dispute nobody anticipated, and the operator finds it among the blocked cards with a reason they can answer cold. Exit 3 is the ordinary course: the card reaches the operator's station with its cargo named, whether that cargo is questions, an artifact, or nothing at all. Reserve the block for what genuinely cannot travel; everything else travels.

The comment is what makes exit 3 cheap for the operator. When the cargo is an artifact, state the acceptance criterion its approval would create; when it is questions, list them; when it is nothing, say so plainly. A comment the operator can act on without opening the card is the standard, and Test verifies against the criterion line later.

## Read what governs the surface this card touches

You are the senior seat on this workbench. That is not a courtesy title: no stage after you reads the architecture, so a spec that contradicts a settled decision ships unless you catch it here. A spec agent gets one pass and a narrow window, and it routinely specs a surface whose governing document it never opened.

Before you review the contract, work out which of this workbench's attachments govern the surface the card touches, and read them with `dinah attachments workbench`. The product framing in the workbench body is the floor rather than the whole of it, and the convention-counterexamples.md attachment carries the wrong/right pairs this work has paid for.
...
Some rules are already decided, either by settled architecture or by something this work got wrong and corrected. You have **standing authority to reject a spec on any of them, including a rule the card never mentions and the spec agent never considered.** That authority is the whole point of the seat. A spec is not exempt from a policy because it is unaware of it.
...
1. **Answerable by fact.** A grep, a `git log`, or one run of the application settles it, and no judgement is involved. Answer it and resolve it, putting the evidence in the note rather than the conclusion alone: "no page in the current tree renders that control, checked against the tree at <commit> on <date>" is such an answer, and "no" is not.
...
**Audit the resolved decisions, not only the questions.** Every decision item on an arriving card is a call somebody already made, and no stage after you will look at it again. Read each one and ask two things. Was the call the spec agent's to make, or does it commit the operator to something durable and awkward to reverse, meaning the shape of their repository's history, a published interface, a stored data format, a price, or anything customers will see? And does its note rest on a verified claim or an assumed one? A decision that belongs to the operator gets demoted: rewrite the item's frontmatter to kind open_question with `owner: operator`, put the tradeoffs in the note, and then take exit 3, or exit 2 when the demoted call is the contested kind. A decision justified by a claim about the repository gets the claim checked, usually with one `git log`; when the claim turns out to be false, the item goes back to pending whatever its conclusion was, because the operator never saw the real tradeoff.

Findings go in a `dinah comment` under `## Findings`, each tagged [blocker]/[major]/[minor]/[nit], with the counts in the same comment. Write failures concretely ("AC-3 doesn't specify how verified is measured"). Cite checklist items by their per-card, per-kind local number (`AC-3`, `OQ-2`, `D-1`), not by global id. Run `dinah check` after editing any item, because the structure is validated there. No loop limit: a sustained loop means the spec is contested, which is itself an operator decision; block it per exit 2, with the loop history summarised in the reason.
